...
export default function Header() {
  ...
  // Fetch notifications (Latest activities)
  useEffect(() => {
    const fetchNotifs = async () => {
      try {
        const res = await api.get('/analytics/dashboard-stats');
        if (res.data.activityLogs) setNotifications(res.data.activityLogs.slice(0, 5));
      } catch (err) {
          ...
          <div className="relative" ref={notifRef}>
            ...
            >
              ...
              {notifications.length > 0 && (
                 <span className="absolute top-3 right-3 w-2 h-2 rounded-full bg-[var(--accent-glow)] shadow-[0_0_10px_rgba(var(--accent-glow),0.5)]" />
              )}
            </button>
            ...
            <AnimatePresence>
              {isNotifOpen && (
                ...
                >
                  <h4 className="text-[10px] font-bold tracking-widest text-[var(--muted-text)] uppercase mb-4 px-2">Intelligence Feed</h4>
                  <div className="space-y-3">
                    {notifications.map((n, i) => (
                      <div key={i} className="flex gap-3 p-2 rounded-xl hover:bg-[var(--nav-active)] transition-colors cursor-pointer">
                        <div className="w-8 h-8 rounded-lg bg-[var(--accent-glow)]/10 border border-[var(--accent-glow)]/20 flex items-center justify-center text-[var(--accent-glow)] shrink-0">
                          {n.type === 'TASK_OK' ? <CheckCircle2 size={14} /> : <Clock size={14} />}
                        </div>
                        <div className="min-w-0">
                          <p className="text-[11px] font-bold text-[var(--primary-text)] leading-tight">{n.message}</p>
                          <p className="text-[9px] text-[var(--muted-text)] mt-1 uppercase font-bold tracking-tight">{new Date(n.timestamp || n.date).toLocaleTimeString()}</p>
                        </div>
                      </div>
                    ))}
                  </div>
